...
type UseBilliardTableColumnsProps = {
  onOpenGallery: (table: BilliardTable) => void;
  permissions: {
    can_update: boolean;
    can_delete: boolean;
  };
...
export function useBilliardTableColumns({ onOpenGallery, permissions }: UseBilliardTableColumnsProps) {
  const { openEdit, openDelete } = useBilliardTableStore();

  return useMemo<ColumnDef<BilliardTable, any>[]>(
    () => [
      {
        id: "no",
        header: "No",
