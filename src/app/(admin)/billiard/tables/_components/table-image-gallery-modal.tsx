...
type TableImageGalleryModalProps = {
  isOpen: boolean;
  onClose: () => void;
  data: GroupedBilliardTableImage | null;
  permissions: {
    can_create: boolean;
    can_delete: boolean;
  };
...
export function TableImageGalleryModal({ isOpen, onClose, data, permissions }: TableImageGalleryModalProps) {
  const { openCreate, openDelete } = useBilliardTableImageStore();
  ...
  return (
    <Modal open={isOpen} onClose={onClose} className="max-w-3xl">
      <ModalHeader className="flex-row items-center justify-between pb-2 border-b border-border mb-4">
        <ModalTitle>Galeri: {data.table_name}</ModalTitle>
      ...
      <ModalBody className="max-h-[70vh] overflow-y-auto no-scrollbar pb-6 space-y-6">
        <div className="flex justify-between items-center">
          <p className="text-sm text-muted-foreground">
            Menampilkan {data.total_images} gambar untuk meja ini.
          </p>
        ...
        {data.total_images === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center border-2 border-dashed border-border rounded-lg bg-muted/20">
            <p className="text-muted-foreground mb-2">Belum ada gambar untuk meja ini.</p>
            {permissions.can_create && (
