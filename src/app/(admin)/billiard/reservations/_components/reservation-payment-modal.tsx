...
import { useReservationPaymentStore } from "@/features/billiard/reservation/store";
...
export function ReservationPaymentModal() {
  const { reservationPayment, closePaymentModal } = useReservationPaymentStore();
  ...
  if (!reservationPayment) return null;

  const executePayment = async () => {
    try {
      const payload = {
        type: "reservation" as const,
        reservation_id: reservationPayment.id,
        gross_amount: reservationPayment.billiard_table.price,
      ...
      if (paymentMethod === "cash") {
        add({
          title: "Berhasil",
          message: "Pembayaran tunai berhasil dicatat. Reservasi Lunas.",
          variant: "success",
        });
          ...
          (window as any).snap.pay(result.snap_token, {
            onSuccess: async function () {
              try {
                ...
                add({
                  title: "Berhasil",
                  message: "Pembayaran QRIS berhasil dan reservasi dilunasi.",
                  variant: "success",
                });
                queryClient.invalidateQueries({ queryKey: ["reservations"] });
                closePaymentModal();
  ...
  return (
    <>
    <Modal
      open={!!reservationPayment}
      onClose={closePaymentModal}
      className="max-w-md"
    >
      <ModalHeader>
        <ModalTitle>Pembayaran Reservasi</ModalTitle>
        <ModalClose onClose={closePaymentModal} />
      </ModalHeader>
      <ModalBody>
        <p className="text-sm text-muted-foreground mb-4">
          Pilih metode pembayaran untuk {reservationPayment.guest_name} (Total: Rp {Number(
            reservationPayment.billiard_table.price
    ...
    <ConfirmModal
      ...
      description="Apakah Anda yakin ingin menyelesaikan reservasi ini dengan pembayaran tunai (Cash)?"
