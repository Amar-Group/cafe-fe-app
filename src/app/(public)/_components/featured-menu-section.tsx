...
const SIGNATURE_ITEMS = [
  {
    name: "Truffle Mushroom Pasta", category: "Main Course",
    description: "Hand-rolled fettuccine tossed in a velvety truffle cream sauce with wild mushrooms, finished with aged parmesan and fresh herbs.",
    image: "/images/cafe/pasta.png", badge: "Chef's Pick", badgeIcon: Award, badgeColor: "bg-cafe-olive text-white",
  },
  {
    name: "Caramel Cloud Latte", category: "Signature Drink",
    description: "Double-shot espresso layered with silky oat milk, house-made caramel, and a cloud of vanilla cream foam.",
    image: "/images/cafe/signature-drink.png", badge: "Best Seller", badgeIcon: Flame, badgeColor: "bg-cafe-orange text-white",
  },
  {
    name: "Berry Crème Brûlée", category: "Dessert",
    description: "Classic French custard with a caramelized sugar crust, served with seasonal berries and matcha roll cake.",
    image: "/images/cafe/dessert.png", badge: "Fan Favorite", badgeIcon: Heart, badgeColor: "bg-rose-500 text-white",
  },
  {
    name: "Yuzu Espresso Tonic", category: "Signature Drink",
    description: "A refreshing fusion of double-shot espresso, zesty Japanese yuzu citrus, and sparkling tonic water over crystal-clear ice.",
    image: "/images/cafe/YUZU_ESPRESSO_TONIC.jpg", badge: "Refreshing", badgeIcon: Flame, badgeColor: "bg-cafe-orange text-white",
  },
  {
    name: "Matcha Espresso Fusion", category: "Signature Drink",
    description: "A bold combination of premium ceremonial grade matcha and rich espresso, layered beautifully with creamy milk.",
    image: "/images/cafe/YUZU_ESPRESSO_TONIC.jpg", badge: "New Arrival", badgeIcon: Flame, badgeColor: "bg-cafe-olive text-white",
  },
  {
    name: "Spicy Prawn Aglio Olio", category: "Main Course",
    description: "Al dente spaghetti tossed in premium olive oil, garlic, chili flakes, and succulent pan-seared tiger prawns.",
    image: "/images/cafe/pasta.png", badge: "Spicy", badgeIcon: Flame, badgeColor: "bg-red-500 text-white",
  },
  {
    name: "Vanilla Oat Cold Brew", category: "Signature Drink",
    description: "24-hour slow-steeped cold brew coffee topped with a smooth, velvety layer of sweetened oat milk foam.",
    image: "/images/cafe/signature-drink.png", badge: "Must Try", badgeIcon: Award, badgeColor: "bg-cafe-olive text-white",
  },
  {
    name: "Classic NY Cheesecake", category: "Dessert",
    description: "Rich and creamy baked New York style cheesecake on a crunchy graham cracker crust, topped with fresh strawberry compote.",
    image: "/images/cafe/dessert.png", badge: "Sweet Treat", badgeIcon: Heart, badgeColor: "bg-rose-500 text-white",
  },
];

// ==========================================
// 2. BILLIARD DATA & PRICING
// ==========================================
...
export function FeaturedMenuSection() {
  ...
  return (
    <section id="venue" ref={ref} className="relative py-24 md:py-32 bg-cafe-charcoal overflow-hidden transition-colors duration-700">
      <div className="absolute inset-0 cafe-noise opacity-50" />

      {/* Dynamic Glow Background based on Tab */}
      <div className={cn(
        "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[500px] rounded-full blur-[150px] pointer-events-none transition-colors duration-1000",
        activeTab === "cafe" ? "bg-cafe-brown/10" : "bg-emerald-500/5"
      )} />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-12 cafe-reveal">
          <span className={cn(
            "inline-block text-xs tracking-[0.4em] uppercase font-semibold mb-4 transition-colors duration-500",
            activeTab === "cafe" ? "text-cafe-orange" : "text-emerald-400"
          )}>
            Our Premium Offerings
          </span>
          <h2 className="font-display text-4xl md:text-5xl lg:text-6xl text-cafe-cream font-light leading-tight mb-8">
            {activeTab === "cafe" ? (
              <>Curated with <em className="font-medium not-italic text-cafe-brown-light">Passion</em></>
            ) : (
              <>Billiard <em className="font-medium not-italic text-emerald-400">Arena</em></>
            )}
          </h2>

          {/* Toggle Button */}
          <div className="inline-flex bg-cafe-dark/80 p-1.5 rounded-full border border-cafe-cream/10 backdrop-blur-md">
            <button
              onClick={() => { setActiveTab("cafe"); setShowAllMenu(false); }}
              className={cn(
                ...
                activeTab === "cafe"
                  ...
                  : "text-cafe-sand/50 hover:text-cafe-cream"
              )}
            >
              Cafe & Resto
            </button>
            <button
              onClick={() => { setActiveTab("billiard"); handleBackToLayout(); }}
              className={cn(
                "px-6 py-2.5 rounded-full text-sm font-semibold tracking-wide transition-all duration-300",
                activeTab === "billiard"
            ...
            <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-8 duration-700">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 cafe-stagger">
                {row1Items.map((item, idx) => (
                  <div
                    key={item.name}
                    className="cafe-reveal group relative bg-cafe-dark/60 backdrop-blur-sm rounded-3xl overflow-hidden border border-cafe-cream/5 hover:border-cafe-cream/15 transition-all duration-500 cursor-pointer"
                    onMouseEnter={() => setHoveredIdx(idx)}
                    onMouseLeave={() => setHoveredIdx(null)}
                  >
                    <div className="relative aspect-[4/3] overflow-hidden">
                      <Image src={item.image} alt={item.name} fill sizes="(max-width: 640px) 100vw, 25vw"
                        className={cn(
                      ...
                      <div className={cn("absolute top-4 left-4 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-semibold tracking-wide uppercase shadow-sm", item.badgeColor)}>
                        <item.badgeIcon size={12} /> {item.badge}
                      </div>
                    </div>
                    <div className="p-6 space-y-3">
                      <span className="text-cafe-brown-light text-xs tracking-[0.2em] uppercase font-medium">{item.category}</span>
                      <h3 className="font-display text-xl text-cafe-cream font-medium">{item.name}</h3>
                      <p className="text-cafe-sand/45 text-sm line-clamp-2 font-light">{item.description}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* Row 2 */}
              {showAllMenu && (
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 animate-in fade-in slide-in-from-bottom-12 duration-1000 fill-mode-both">
                  {row2Items.map((item, idx) => {
                    const actualIdx = idx + 4;
                    return (
                      <div
                        key={item.name}
                        className="group relative bg-cafe-dark/60 backdrop-blur-sm rounded-3xl overflow-hidden border border-cafe-cream/5 hover:border-cafe-cream/15 transition-all duration-500 cursor-pointer"
                      ...
                      >
                        <div className="relative aspect-[4/3] overflow-hidden">
                          <Image src={item.image} alt={item.name} fill sizes="(max-width: 640px) 100vw, 25vw"
                            className={cn(
                              "object-cover transition-transform duration-700",
                              hoveredIdx === actualIdx ? "scale-110" : "scale-100"
                            )}
                          />
                          <div className="absolute inset-0 bg-gradient-to-t from-cafe-dark/80 via-transparent to-transparent" />
                          <div className={cn("absolute top-4 left-4 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-semibold tracking-wide uppercase shadow-sm", item.badgeColor)}>
              ...
              {apiSignatureItems.length > 4 && !showAllMenu && (
                <div className="text-center mt-10">
                  <button
                    ...
                    className="group inline-flex items-center gap-2 px-8 py-3 bg-transparent border-2 border-cafe-cream/20 text-cafe-cream rounded-full text-sm font-semibold tracking-wide hover:bg-cafe-cream hover:text-cafe-charcoal hover:border-cafe-cream transition-all duration-300"
                  >
                    View Full Menu
                  ...
                  {step === "form" && (
                    <form onSubmit={handleBookingSubmit} className="space-y-8">
                      <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out fill-mode-both">
                        <span className="text-emerald-400/80 text-xs tracking-[0.3em] uppercase font-semibold block mb-2">
                      ...
                      <button
                        ...
                        className={cn(
                          "w-full py-4 rounded-2xl font-bold uppercase tracking-widest text-xs transition-all duration-200 mt-4 shadow-xl animate-in fade-in slide-in-from-bottom-4 delay-500 duration-700 ease-out fill-mode-both",
